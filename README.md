...
Hooks that take concepts from RxJS and Svelte to make handling state in React really simple and powerful.
...
subscribers when its value actually changes. If `next()` is invoked twice with different but identical objects, the second invocation will be ignored and subscribers will not be notified.

Also included in this library is a set of React hooks that make interacting with a store
...
if you set the `immutable` option to `false` (this tells the store that you will not be passing it immutable objects):
```typescript
const initialState = { foo: 'bar', more: 'state' }
...
```
This will work on any objects that are Cloneable according to the lodash.clone() documentation.
...
Note that `this.value` contains the current version of the store. That is inherited from `BehaviorSubject`.

### Asynchronous mutations
Mutation methods become even more powerful when you do asynchronous updates. All of your asynchronous code can be encapsulated in a single method, making it much easier to think about how you want to handle race conditions and errors.
```typescript
interface BookState {
  loading?: boolean
  book?: Book
...
the state from the store and trigger a render every time it changes. Let's look at using the BookStore from the example above:
```tsx
...
const SimpleComponent: React.FC = props => {
  ...
  const onClick = useCallback(() => updateSimpleState(simpleState.count + 1), [simpleState, updateSimpleState])
  return <>
...
## Advanced Usage / Tips
### lodash.get & set
Anything in this library that accepts a property name uses `lodash.get` and `lodash.set` so that you can use dot-notation to go deeper into your state object.
```tsx
...
```
### BehaviorSubject
The Store class extends BehaviorSubject from RxJS. So anything you can do with a BehaviorSubject you can do with a store, including subscribing without a hook, piping, etc.
```typescript
...
```typescript
import { DerivedStore } from 'react-store-hooks'
...
Everything is exported as an es6 module, which should be helpful for any React project since even pure javascript React projects use a precompiler like webpack.
